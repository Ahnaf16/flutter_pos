...
class _RenderOverflowMarqueeLayout extends RenderShiftedBox
  ...
  double get offsetProgress {
    final double durationInMicros = duration.inMicroseconds * ((sizeDiff ?? 0) / step);
    final int delayDurationInMicros = delayDuration.inMicroseconds;
    double elapsedInMicros = elapsed.inMicroseconds.toDouble();
    // includes the reverse
    final double overalCycleDuration =
    ...
    final bool reverse = elapsedInMicros > delayDurationInMicros + durationInMicros;
    final double cycleElapsedInMicros = elapsedInMicros % (delayDurationInMicros + durationInMicros);
    if (cycleElapsedInMicros < delayDurationInMicros) {
      return reverse ? 1 : 0;
    } else if (cycleElapsedInMicros < delayDurationInMicros + durationInMicros) {
      final double progress = (cycleElapsedInMicros - delayDurationInMicros) / durationInMicros;
  ...
  double get fadeStartProgress {
    final child = this.child;
    if (child != null) {
      final double size = sizeDiff ?? 0;
      final double progressedSize = size * offsetProgress;
  ...
  double get fadeEndProgress {
    ...
    if (child != null) {
      final double size = sizeDiff ?? 0;
      final double progressedSize = size * (1 - offsetProgress);
      return (progressedSize / fadePortion).clamp(0, 1);
    }
    return 0;
  }
  ...
  Shader _createAlphaShader(bool fadeStart, bool fadeEnd, Rect bounds, double fadePortion) {
    ...
    if (fadeStart) {
      final double start = fadeStartProgress;
      final Color startColor = Colors.white.withValues(alpha: 1 - start);
    ...
    if (fadeEnd) {
      final double end = fadeEndProgress;
      final Color endColor = Colors.white.withValues(alpha: 1 - end);
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (child != null) {
      ...
      final Shader shader = _createAlphaShader(
